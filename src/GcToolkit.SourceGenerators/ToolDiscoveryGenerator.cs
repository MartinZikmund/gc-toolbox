...
public sealed class ToolDiscoveryGenerator : IIncrementalGenerator
{
        ...
        string rootNamespace)
    {
        ...
        context.AddSource("GeneratedToolCatalog.g.cs", EmitCatalog(result, rootNamespace));
        context.AddSource("ToolDiscoveryServiceCollectionExtensions.g.cs", EmitServiceCollection(result, rootNamespace));
        context.AddSource("ViewRegistrations.g.cs", EmitViewRegistrations(result, leafViews, rootNamespace));
    }
    ...
    private static SourceText EmitCatalog(DiscoveryResult result, string rootNamespace)
    {
        ...
        foreach (var tool in result.Tools)
        {
            ...
            // Discovered tools have no dedicated view yet; they render in the shared host as placeholders (R3).
            sb.AppendLine("            true,");
    ...
    private static SourceText EmitViewRegistrations(DiscoveryResult result, IReadOnlyList<ViewModel> leafViews, string rootNamespace)
    {
        var sb = new StringBuilder();
        sb.AppendAutoGeneratedHeader();
        sb.AppendLine($"namespace {rootNamespace};");
        sb.AppendLine();
        sb.AppendLine("/// <summary>Registers every discovered view↔ViewModel pair, plus each tool ViewModel → the shared ToolHostView.</summary>");
        ...
        {
            sb.AppendLine();
            sb.AppendLine("        // Discovered tool ViewModels share the tool host view (R10).");
            foreach (var tool in result.Tools)
            {
